...
export class ApiDoc extends React.Component<ApiDocProps, ApiDocState> {
  ...
  renderPropertyUsage = (usage: PropertyUsage, index: number) => {
    return <div key={index} className={classNames('example', {block: usage.value.indexOf('\n') > 1})}>
  ...
  renderProperty = (property: Property, index: number) => {
    ...
    return <div className="property" key={index}>
      ...
      <div className="description">{property.description.content ? property.description.content : <i>no description</i>}</div>
      {examples.length > 0
  ...
  render() {
    ...
    return <div className="hy-api-doc hy-section">
      <div className="title">API</div>
      <div className="content">
        <div className="label">{doc.name}</div>
        <div className="properties">
          {doc.properties.map(this.renderProperty)}
        </div>
      </div>
    </div>;
  }
